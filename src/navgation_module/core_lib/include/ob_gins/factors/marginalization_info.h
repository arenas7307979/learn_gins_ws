...
#include "ob_gins/factors/residual_block_info.h"
...
public:
    ...
    static int globalSize(int size) {
        std::cout << "May have error pose == babg == 6dimension" <<std::endl;
        exit(-1);
...
private:
    ...
    void linearization() {
        ...
        // std::cout << "linearized_residuals_=" << linearized_residuals_ <<std::endl;
        // std::cout << "linearized_jacobians_=" << linearized_jacobians_ <<std::endl;
    }
    ...
    bool updateParameterBlocksIndex() {
        ...
        for (auto &block : parameter_block_index_) {
            ...
            // std::cout << "local size=" << localSize(parameter_block_size_[block.first]) <<std::endl;
        }
