...
public:
  ...
  explicit EdgeImu(IntegrationBasePtr _pre_integration,
...
private:
  IntegrationBasePtr pre_integration_;
  Eigen::Vector3d Gw; // just the gravity term between the imu
};

class EdgeImuTd : public Edge {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    explicit EdgeImuTd(IntegrationBasePtr _pre_integration, const Eigen::Vector3d &Gw_, const double &last_estimate_td_)
    ...
    /// 返回边的类型信息
    virtual std::string TypeInfo() const override { return "EdgeImuTd"; }
