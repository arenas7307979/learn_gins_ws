...
struct number_backend_float_architype
{
...
#endif
      std::cout << "const char* Assignment (" << s << ")" << '\n';
      return *this;
   }
   ...
   std::string str(std::streamsize digits, std::ios_base::fmtflags f)const
   {
      ...
      if(!(f & std::ios_base::scientific) && m_value == i)
         ...
         ss << m_value;
      std::string s = ss.str();
      std::cout << "Converting to string (" << s << ")" << '\n';
      return s;
   }
   void negate()
   {
      std::cout << "Negating (" << m_value << ")" << '\n';
      m_value = -m_value;
   }
   ...
   {
      std::cout << "Comparison with int" << '\n';
      return m_value > i ? 1 : (m_value < i ? -1 : 0);
   }
   int compare(boost::ulong_long_type i)const
   {
      std::cout << "Comparison with unsigned" << '\n';
      return m_value > i ? 1 : (m_value < i ? -1 : 0);
   }
   int compare(long double d)const
   {
      std::cout << "Comparison with long double" << '\n';
