...
class YAML_CPP_API Exception : public std::runtime_error {
 public:
  Exception(const Mark& mark_, const std::string& msg_,const std::string &key_="")
      : std::runtime_error(build_what(mark_, msg_, key_)), mark(mark_), msg(msg_) {}
 ...
 private:
  static const std::string build_what(const Mark& mark,
    ...
    output << "yaml-cpp: " << key_name << " error at line " << mark.line + 1 << ", column "
           << mark.column + 1 << ": " << msg;
    return output.str();
  }
};
...
class YAML_CPP_API RepresentationException : public Exception {
 public:
  RepresentationException(const Mark& mark_, 
                          const std::string& msg_,
                          const std::string& key_="")
      : Exception(mark_, msg_,key_) {}
...
class YAML_CPP_API BadConversion : public RepresentationException {
 public:
  explicit BadConversion(const Mark& mark_,const std::string &key="")
      : RepresentationException(mark_, ErrorMsg::BAD_CONVERSION,key) {}
...
class TypedBadConversion : public BadConversion {
 public:
  explicit TypedBadConversion(const Mark& mark_,const std::string &key="") 
    : BadConversion(mark_,key) {}
};
