...
T temme_method_1_ibeta_inverse(T a, T b, T z, const Policy& pol)
{
   ...
   std::cout << "Estimating x with Temme method 1: " << x << '\n';
#endif
   return x;
}
//
...
#ifdef BOOST_INSTRUMENT
      std::cout << "Estimating x with Temme method 2 (small eta): " << x << '\n';
#endif
   }
   else
   {
      ...
      if((x - s_2) * eta < 0)
         x = 1 - x;
#ifdef BOOST_INSTRUMENT
      std::cout << "Estimating x with Temme method 2 (large eta): " << x << '\n';
#endif
   }
   //
...
#ifdef BOOST_INSTRUMENT
   std::cout << "Estimating x with Temme method 3: " << x << '\n';
#endif
   return x;
}
