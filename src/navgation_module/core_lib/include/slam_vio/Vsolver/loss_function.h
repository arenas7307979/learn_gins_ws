...
//
#ifndef VSOLVER_LOSS_FUNCTION_H
#define VSOLVER_LOSS_FUNCTION_H
...
namespace Vsolver {
    ...
    {
    public:
        explicit CauchyLoss(double delta = 1) : delta_(delta), dsqr(delta*delta), dsqrReci(1./(delta*delta)) {}

        virtual void Compute(double err2, Eigen::Vector3d& rho) const override;

    private:
        double delta_; //c = lossfunction of ceres:a
        double dsqr;// c^2 = lossfunction of ceres:b 
        double dsqrReci ;// 1/c^2 = lossfunction of ceres:c
    };
