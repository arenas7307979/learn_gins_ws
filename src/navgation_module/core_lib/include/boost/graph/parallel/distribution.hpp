...
struct twod_block_cyclic
{
  ...
  SizeType block_size(ProcessID id, SizeType n) const
  {
    ...
    //    std::cerr << "Block size of id " << id << " is " << result << '\n';
    return result;
  }
  ...
  { 
    ...
    //    std::cerr << "Item " << i << " goes on processor " << result << '\n';
    return result;
  }
  ...
  { 
    ...
    //    std::cerr << "Item " << i << " is in block #" << block_num << '\n';
    ...
    //    std::cerr << "Item " << i << " maps to local index " << block_start+block_offset << '\n';
