...
public:
                                      ...
                                      test_results const *tr) const
    {
        ...
        if(!detailed_log.system_err.empty() || has_failed)
        {
            ...
            if(tu->p_type == TUT_SUITE) {
                if( tu->p_id == m_ts.p_id ) {
                    o << " boost.test global setup/teardown" << '\n';
                } else {
                    o << "- test suite: " << tu_name_remove_newlines(tu->full_name()) << '\n';
                }
            }
            else {
    ...
    bool    test_suite_start( test_suite const& ts )
    {
        ...
        if( m_ts.p_id == ts.p_id ) {
            ...
            {
                m_stream  << "<properties>" << '\n';
                m_stream  << "<property name=\"platform\" value" << utils::attr_value() << BOOST_PLATFORM << '\n';
                m_stream  << "<property name=\"compiler\" value" << utils::attr_value() << BOOST_COMPILER << '\n';
                m_stream  << "<property name=\"stl\" value" << utils::attr_value() << BOOST_STDLIB << '\n';

                std::ostringstream o;
                o << BOOST_VERSION/100000 << "." << BOOST_VERSION/100 % 1000 << "." << BOOST_VERSION % 100;
                m_stream  << "<property name=\"boost\" value" << utils::attr_value() << o.str() << '\n';
                m_stream  << "</properties>" << '\n';
            }
        }
